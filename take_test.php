<?php 
	...
	if($db->connect_error) {
		...
		session_start();
		$datas = [];
		if($_SESSION['test']=='Random')
		{
			$category = $_SESSION['category'];
			$sql = "SELECT * FROM question WHERE CATEGORY='$category' ORDER BY RAND() LIMIT 15;";
		}

		else if(isset($_POST['topics']))
		{
			$test = implode("', '", $_POST['topics']);
			$sql = "SELECT * FROM question WHERE TEST_NAME IN ('$test') ORDER BY RAND() LIMIT 15;";
			$_SESSION['test'] = $test;			
		}
		else {

			$test = $_SESSION['test'];
			$sql = "SELECT * FROM question WHERE TEST_NAME=$test ORDER BY RAND() LIMIT 15;";
		}
...
<html>
	...
	<script type="text/javascript">
		...
		function nextPage(numCorrect){
			window.location.href="results.php?score="+numCorrect;
		}
